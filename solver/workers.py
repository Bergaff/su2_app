...
def parse_history(case_dir: str):
    ...
    try:
        ...
        icd = col("cd")
        icm = col("cmz", "cm", "cmy")
        # Сжимаемый решатель пишет rms[Rho]; несжимаемый (INC_*) не имеет
        # уравнения плотности — его колонки rms[P], rms[Ux], rms[Uy],
        # rms[Uz]. Без колонок INC прогон INC_EULER сходился до -8 по
        # давлению, а в лог писалось log10(rms[Rho])=None, и сравнение
        # прогонов шло вслепую.
        irms = col("rms[rho]", "rms[p]", "rms[pressure]", "res_rms_rho",
                   "rms[res_rho]")
...
class SessionRunner(QThread):
    ...
    def _usable(r):
        ...
        if not has_coeff:
            return False
        # Ненулевые коэффициенты есть ещё и у разошедшегося прогона:
        # SU2 пишет history.csv по ходу счёта, и строка на пике вылета
        # содержит «мгновенные» CL/CD (замер на этой сетке: Cd = -0.05 при
        # log10(rms) = +0.05 — невязка не упала ни на порядок). Такой
        # прогон результатом не считается: невязка обязана упасть хотя бы
        # на порядок. Прогон на плато (log10(rms) = -2.3 при норме -7)
        # проходит — плато это не расходимость.
        rms = r.get("rms")
        if isinstance(rms, (int, float)) and float(rms) > -1.0:
            return False
        # Отрицательное сопротивление полного самолёта физически
        # невозможно (замер на этой сетке: второй порядок при log10(rms)
        # = -2.7 дал Cd = -0.0147 — недоразрешённая носка крыла даёт
        # «численную тягу»). Такой прогон в итог не идёт, как бы глубоко
        # ни упала невязка: пользователь получит отрицательное Cd и
        # отрицательное качество вместо честного «схема не справилась».
        cd = r.get("cd")
        if isinstance(cd, (int, float)) and float(cd) < -5e-3:
            return False
        return True
    ...
    def _better_result(prev, new, log_cb):
        ...
        # И ХОТЬ ЧТО-ТО СОШЁЛСЯ. Замер на телооблекающей сетке (до правки
        # сетки): второй порядок расходился с невязкой 10^+0.05, его
        # «мгновенные» коэффициенты (Cd = -0.05, L/D = 0 — физически
        # невозможные) выигрывали у сошедшегося первого порядка только
        # потому, что он «второго порядка». Предпочтение схемы действует,
        # лишь если невязка второго порядка упала хотя бы на два порядка
        # (log10 <= -2): ниже этого порога «точность схемы» не имеет
        # смысла — коэффициенты не установились.
        _rms_2o = prev.get("rms")
        _conv_2o = (isinstance(_rms_2o, (int, float)) and _rms_2o <= -2.0)
        ...
        if not SessionRunner._usable(prev):
            if SessionRunner._usable(new):
                _pcd = prev.get("cd")
                if (prev.get("second_order") is True
                        and isinstance(_pcd, (int, float))
                        and float(_pcd) < -5e-3):
                    log_cb("Внимание: второй порядок дал отрицательное "
                           "сопротивление (Cd=%+.4f) — физически "
                           "невозможно, это численный артефакт грубой "
                           "сетки (недоразрешённая носка крыла даёт "
                           "«тягу»). Такой результат в отчёт не идёт. "
                           "Оставлен %s; его Cd завышен схемной вязкостью "
                           "— коэффициенты приблизительные."
                           % (float(_pcd),
                              "2-й порядок" if new.get("second_order")
                              else "1-й порядок"))
                elif prev.get("second_order") is True and \
                        isinstance(_rms_2o, (int, float)):
                    log_cb("Внимание: второй порядок не сошёлся "
                           "(log10(rms)=%+.2f — невязка не упала даже на "
                           "порядок), его коэффициенты не установились и в "
                           "результат не идут. Оставлен повтор (%s); "
                           "коэффициенты приблизительные: схемная вязкость "
                           "первого порядка завышает Cd."
                           % (_rms_2o,
                              "2-й порядок" if new.get("second_order")
                              else "1-й порядок"))
                else:
                    log_cb("Внимание: прогон вторым порядком разошёлся и не "
                           "дал коэффициентов; оставлен повтор (%s). "
                           "Коэффициенты приблизительные: порядок схемы "
                           "снижен."
                           % ("2-й порядок" if new.get("second_order")
                              else "1-й порядок"))
            return new
