...
# Значения выведены из восьми официальных кейсов SU2 (TestCases/euler/*
# и TestCases/navierstokes/*, ветка master). Что в них повторяется:
#   CFL_ADAPT            — NO либо не задан (по умолчанию false): 8 из 8.
...
# и использует LOW_MACH_PREC= YES при фиксированном CFL 1.0.
PRESETS = {
    # «Быстро» (safe): первый порядок. Сходится на плохой сетке, но схемная
    # вязкость велика и Cd читать нельзя — годится для прикидки Cl.
    "safe": {
    ...
    # конфига solver/config_builder.py, поэтому apply_preset его фактически
...
def describe_config_scheme(path):
    ...
    if prec:
        ...
        lm = (" ВНИМАНИЕ: M=%.3f < 0.3, а LOW_MACH_PREC= NO — невязка на "
              "таком режиме падает неохотно, а Cd завышен схемой Роу. "
              "Прекондиционер можно включить в настройках SU2, но на этой "
              "модели он уже проверен и дал расходимость, поэтому по "
              "умолчанию выключен." % mach)
    else:
        lm = ""
