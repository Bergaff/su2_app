...
PARTITION_HELP = (
    "<b>SU2_PARTITION</b> — это отдельная программа комплекса SU2, "
    "которая <b>режет сетку на части</b> для параллельного расчёта "
    "на нескольких ядрах/процессах (запуск через <code>mpirun -np N "
    "SU2_CFD.exe</code>).\n\n"

    "<b>Если ты запускаешь обычный одиночный SU2_CFD.exe</b> (без mpirun) "
    "— партиционер не нужен: расчёт идёт в одном процессе на одном ядре. "
    "Предупреждение «SU2_PARTITION не найден — auto-partition» в этом "
    "случае <b>безвредно</b>, на результат не влияет, можно игнорировать.\n\n"

    "<b>Когда он нужен:</b> сетки ~1 млн ячеек и больше считаются на одном "
    "ядре медленно (твой прогон — 6.5 минут на сотни итераций). Чтобы "
    "ускорить в N раз, нужна <b>MPI-сборка SU2</b>: файлы SU2_CFD.exe и "
    "SU2_PARTITION.exe, собранные с поддержкой MPI, плюс установленный "
    "Microsoft MPI (msmpisetup.exe). Тогда запуск выглядит как "
    "<code>mpiexec -np 4 SU2_CFD.exe config.cfg</code>, а партиционер "
    "делит mesh.su2 на 4 части автоматически.\n\n"

    "Итог: сейчас это просто информационное сообщение приложения. Хочешь "
    "быстрее считать большие сетки — поставим параллельную сборку SU2; "
    "для отладки и обычных задач хватает последовательной."
)
