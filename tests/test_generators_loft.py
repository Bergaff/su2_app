...
print()
print("== киль: инлайн-копия в UI давала плоский лист ==")
# ui/main_window.py собирал киль своей копией кода, где толщина профиля
# уходила в Z, а Y обоих сечений был жёстко нулём. Готовый генератор
# generate_vertical_stabilizer_geometry это уже умел правильно, но UI его
# не вызывал. Параметры ниже — из реального лога пользователя:
# «ВО: H=1.2м, chord=0.70м», в mesh.su2 уходило v_stabilizer.stl (316 граней)
# с мин. габаритом 0.0000 м.
vk = gen.generate_vertical_stabilizer_geometry(
    _AM(), "NACA0012", 1.2, 0.70, 0.70 * 0.45, 30.0, z_offset=0.55)
vkv, vkf = _tri(vk)
vb = vkv.min(axis=0), vkv.max(axis=0)
vdy = float(vb[1][1] - vb[0][1])
vdz = float(vb[1][2] - vb[0][2])
check("киль: толщина по Y не нулевая (%.4f м)" % vdy, vdy > 1e-6,
      "было ровно 0.0000")
check("киль: толщина по Y равна 12%% хорды (%.4f против %.4f)"
      % (vdy, 0.12 * 0.70), abs(vdy - 0.12 * 0.70) < 1e-3)
check("киль: высота по Z равна заданной, а не раздута толщиной "
      "(%.4f против 1.2000)" % vdz, abs(vdz - 1.2) < 1e-6,
      "было 1.2609")
# Инлайн-копия отдавала 316 граней: по одному поясу на пролёт, то есть
# панели высотой 1.2 м при хордовом шаге 0.009 м. Генератор вставляет
# промежуточные сечения, поверхность та же, а триангуляция мельче.
vke = _edges(vkv, vkf)
check("киль: граней больше, чем давал инлайн (%d против 316)" % len(vkf),
      len(vkf) > 316)
check("киль: наибольшее ребро не в полтора метра (%.4f)" % vke.max(),
      vke.max() < 0.40)
if HAS_TRIMESH:
    vkt = trimesh.Trimesh(vertices=vkv, faces=vkf, process=False)
    check("киль: это объём, а не лист", vkt.is_volume is True)
    check("киль: поверхность замкнута", vkt.is_watertight is True)
    # 0.026599 м³ — измерено и для генератора, и для инлайн-копии с
    # исправленной толщиной: форма та же. Разница в том, что инлайн-копия
    # даже с толщиной даёт is_volume=False из-за несогласованной
    # ориентации граней, а такая поверхность ломает булево объединение
    # тел и TetGen.
    check("киль: объём сошёлся (%.6f против 0.026599)" % vkt.volume,
          abs(vkt.volume - 0.026599) < 1e-5)

# Дубликат геометрии в UI и был причиной: проверить, что киль теперь
# берётся из генератора, а не собирается заново.
_ui = open(os.path.join(_ROOT, "ui", "main_window.py"), encoding="utf-8").read()
check("UI берёт киль из генератора",
      "generate_vertical_stabilizer_geometry(" in _ui)
# Именно эта строка была подписью инлайн-киля: концевое сечение
# сдвинуто по Z на высоту, а Y у обоих сечений ноль. У ГО похожая строка
# законна — там корневое сечение лежит на оси симметрии.
check("в UI не осталось инлайн-копии киля",
      "tz[i] + pos_z + height" not in _ui)
